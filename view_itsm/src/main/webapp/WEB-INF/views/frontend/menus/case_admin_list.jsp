...
// 								if ( gmGetStep == "alt_05" && gmGetStep != admin_level_type_code ) {
								if ( gmGetStep =="OPEN" ) {
									...
									if ( "OPEN" != textUDN ) {
										var page_link_u = page_link.replace('case_admin_list', 'new_1');
...
// 								comment = $("#new_appr_req_comment").val();
								page_link = page_link.replace('case_admin_list', 'appr_admin_list');
					...
					success : function( data ) {
						if(data.status == 200 ) {
							...
							if ( (status == "MD1267" || status == "MD1268") && requester_id != member_id ) { // 승인자가 요청된 승인을 저장할 때
								page_link = page_link.replace('case_admin_list','new_1');
			...
			onSelect: function (event) {
				...
				event.onComplete = function () {
			        ...
			        if (sel.length) {
			            ...
			            // 내 로컬 주소 복사
// 			            $("#input_text").val("http://localhost:8080/frontend/location/menus/case_admin_list?request_seq=" + encodeURIComponent(getData.request_seq));
			            
			            // 개발 서버 주소 복사
// 			            $("#input_text").val("http://10.1.1.172:8080/frontend/location/menus/case_admin_list?request_seq=" + encodeURIComponent(getData.request_seq));
			            
			            // 운영 서버 주소 복사
// 			            $("#input_text").val("https://itss.vieworks.com/frontend/location/menus/case_admin_list?request_seq=" + encodeURIComponent(getData.request_seq));
			           
			            $("#input_text").val(window.location.origin+"/frontend/location/menus/case_admin_list?request_seq=" + encodeURIComponent(getData.request_seq));
			            
// 			            $("#input_text").val("http://localhost:8080/frontend/location/menus/case_admin_list?request_id=" + encodeURIComponent(getData.request_id));
// 			            $("#textArea").text("/frontend/location/menus/case_admin_list?request_seq=" + encodeURIComponent(getData.request_seq));
...
//        	        						$("#hiddenRow").val(gmData.request_id);
        			    				...
        			    				fnSelRequest("category", getData.request_type);
    	         			            ...
    	         			            $("#hiddenRow").val(getData.request_id);
//     	         			           $("#input_text").val("http://localhost:8080/frontend/location/menus/case_admin_list?request_seq=" + encodeURIComponent(mainData.request_seq));
    	       			            
    	       			            // 개발 서버 주소 복사
//    	        			            $("#input_text").val("http://10.1.1.172:8080/frontend/location/menus/case_admin_list?request_seq=" + encodeURIComponent(mainData.request_seq));
    	       			            
    	       			            // 운영 서버 주소 복사
   	        			            $("#input_text").val("https://itss.vieworks.com/frontend/location/menus/case_admin_list?request_seq=" + encodeURIComponent(mainData.request_seq));
...
// 		window.location.href = "/frontend/location/menus/case_admin_list?project_no=" + encodeURIComponent(키값);
