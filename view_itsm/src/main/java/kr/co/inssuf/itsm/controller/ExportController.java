...
import kr.co.inssuf.itsm.svc.LogStatusService;
import kr.co.inssuf.itsm.util.ExportExcelUtil;
...
public class ExportController {
	...
	@Resource(name = "itsmLogStatusService")
	...
	// Case처리에서 엑셀을 다운 받을 때 헤더부분만 표시하기 위해 새롭게 만든 함수
	@RequestMapping(value = "/export_client_Case_jqgrid", method = RequestMethod.POST, produces="text/html;charset=UTF-8")
	@ResponseBody
	@SuppressWarnings({ "unchecked", "unused" })
	public String goExportCaseClientJQGrid(HttpServletRequest request, HttpServletResponse response) throws Exception {
		logger.info("ExportController.goExportCaseClientJQGrid() is called. ");
		JSONObject resultData = new JSONObject();
		...
		try {
			...
			result = ExportExcelUtil.createCaseAdminXls(filePath, ht_header, listBodyData);
			if(result){
				resultData.put("status", HttpStatus.OK.value());
				resultData.put("file_path", return_filePath);
				resultData.put("file_name", fileName);
