package kr.co.inssuf.itsm.util;
...
//									System.out.println(col_id[j] + " : " + c_val);
	...
	@SuppressWarnings("unused") // Case 처리(CaseAdminList) 에서 엑셀을 다운 받을 때 헤더만 노란색으로 표시해줌
	public static boolean createCaseAdminXls(String file_output, Hashtable<String, Object> opt, JSONArray listBodyData){
		boolean result = false;
		XSSFWorkbook workbook = null;
		// HSSFWorkbook workbook = null;
		FileOutputStream fs = null;
...
//			System.out.println(listBodyData.toJSONString());
			...
			if(listBodyData != null && listBodyData.size() > 0) {
				for(Object obj: listBodyData){
			  		JSONObject nobj = (JSONObject) obj;
			  		Row row = sheet.createRow(rownum++);
			  		System.out.println("createCaseAdminXls 에서 value3 확인 : "+nobj.get("value3"));
			  		if(nobj.get("value3").equals("header")){
			  			for (int j = 0; j < col_name.length; j++) {
				  			Cell cell = row.createCell(j);
				  			String colId = col_id[j].toString();
				  			if(col_id[j] != null && colId.endsWith("_amt") || col_align[j].equals("right")){
...
//				  			} 
				  				...
				  				cell.setCellStyle(styles.get("cell_header"));
				  			}
				  		}
			  		}
			  		else{
...
//									System.out.println(col_id[j] + " : " + c_val);
	...
	private static Map<String, CellStyle> createStyles(Workbook wb){
	    ...
	    style.setBorderBottom(CellStyle.BORDER_THIN);
	    style.setBottomBorderColor(IndexedColors.BLACK.getIndex());
	    style.setFillForegroundColor(IndexedColors.YELLOW.getIndex()); // 색상 관련 (노란색)
	    style.setFillPattern(CellStyle.SOLID_FOREGROUND); // 색상 관련 추가해야 됨
	    style.setFont(bodyFont);
	    styles.put("cell_header", style);
